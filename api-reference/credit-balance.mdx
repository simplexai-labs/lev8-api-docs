...
[`GET /v1/credit/balance`](/api-reference/credit-balance/get-balance) returns the credits that the API key's billing account can use for a new request.

## Headers

| Header | Required | Description |
| --- | --- | --- |
| `Authorization` | Yes | `Bearer lev8_live_...` — any active Lev8 API key. Entity Search scope is not required. |
...
response = httpx.get(
    ...
    headers={"Authorization": f"Bearer {os.environ['LEV8_API_KEY']}"},
    timeout=30,
)
...
| `503` | `service_unavailable` | The API is not enabled in this environment. |
